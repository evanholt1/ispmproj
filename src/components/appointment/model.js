...
const AppointmentSchema = new Schema(
    {
        ...
        state: {
            // blood collection,hemodialysis, medicine delivery, home medical services have different states.
...
var PreAutoPopulate = function (next) {
    this.populate({ path: "allocatedStaff", select: "name" }).populate({ path: "user", select: "name" });
    next();
};

AppointmentSchema.pre("findOne", PreAutoPopulate).pre("find", PreAutoPopulate);
...
const revertState = function (currentState, service) {
    switch (service) {
        case "Hemodialysis Request":
            if (currentState > -1 && currentState != 3) currentState -= 1;
            break;
        default:
            if (currentState > -1 && currentState != 5) currentState -= 1;
            break;
    }
    return currentState;
};
...
function updateState(StateInput) {
    ...
    if (!this.state || StateInput == 0) return 0;
