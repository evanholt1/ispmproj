...
EmployeeSchema.statics.findFreeStaff = async function (serviceDepartments, date) {
    ...
    const hourBeforeDate = appointmentDate.minus({ hours: 1 }),
        ...
        dateHourandMinute = parseInt(appointmentHour.toString() + appointmentMinute.toString());
    ...
    let areEmployeesFromAllDepartmentsAvailable = serviceDepartments.every((service) =>
        freeEmployees.some((employee) => employee._id == service)
    );
